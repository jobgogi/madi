-- 카테고리 enum·배치 문장 개수 일치·출력 형식 지시는 코드(lib/providers/analyze.ts의
-- LOCKED_PROMPT_RULES, resolvePromptTemplate)로 옮겨서 관리자가 prompt_templates에서
-- 수정할 수 없게 고정했다 (2026-09-11 사용자 요청 — "JSON 관련 부분은 아무도 수정 못
--하게"). 기존에 시드했던 v1 두 행의 content에서 그 세 섹션을 제거해 중복되지 않게 한다.
